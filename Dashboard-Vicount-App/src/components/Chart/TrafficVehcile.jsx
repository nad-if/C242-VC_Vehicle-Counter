...
const TrafficVehicle = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await axios.get('https://v-count-api-660543800145.asia-southeast1.run.app/api/vehicle');
  ...
  return (
    <div className="bg-white shadow-lg rounded-xl p-6 lg:col-span-2">
      <div className="mb-4 flex flex-wrap sm:flex-nowrap items-center justify-between">
      <h2 className="text-xl font-semibold mb-4">Traffic Vehicle Trend</h2>
      <button
        onClick={fetchData}
        className="mb-4 text-blue-600 rounded-lg"
      >
        <RotateCw />
      </button>
      </div>
